...
import { Input } from '../ui/Input';
 // Keep if we switch back, or remove. I'll remove for now but the file uses <textarea> directly for full control
// Actually it is unused currently as I used native <textarea>. Let's remove it.
import { Card } from '../ui/Card';
import { Bold, Italic, List, Link as LinkIcon, Variable, Eye, ChevronLeft, ChevronRight, Hash } from 'lucide-react';
...
export function MarkdownEditor() {
  ...
  // Helper to extract available variables from the first recipient
  ...
  const availableVariables = React.useMemo(() => {
    ...
    if (state.recipients.length > 0) {
      ...
      return Array.from(new Set([...standard, ...Object.keys(first).filter(k=>k!=='id')]));
    }
    return standard;
  }, [state.recipients]);
  ...
  const insertVariable = (varName: string) => {
    ...
    // Reset focus and cursor slightly delayed to allow react render
    setTimeout(() => {
        textarea.focus();
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
       <div>
           ...
           <p className="text-slate-500">Draft your message using Markdown and personalized variables.</p>
       </div>
       ...
       <div className="space-y-4 flex-1 flex flex-col">
            ...
            <Card className="flex-1 flex flex-col overflow-hidden shadow-sm border-slate-300">
                ...
                <div className="flex items-center gap-1 p-2 border-b border-slate-200 bg-slate-50">
                    <Button variant="ghost" size="sm" onClick={() => insertMarkdown('h1')} title="Heading">
                    ...
                    {showPreview ? (
                        ...
                        />
                         ...
                         <textarea
                            ...
                            onChange={(e) => dispatch({ type: 'UPDATE_EMAIL_CONTENT', payload: e.target.value })} 
                        />
                    )}
                </div>
            </Card>
